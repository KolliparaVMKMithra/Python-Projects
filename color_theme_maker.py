import tkinter
from tkinter import BOTH, TRUE,IntVar,filedialog
...
def get_red(slider_value):
    ...
    #turn slider value into an int and hex value
...
def store_color():
    ...
    new_color_black_box = tkinter.Label(output_frame,text='black',width=3,height=1)
    ...
    new_color_black_box.grid(row=stored_color.get(),column=4,padx=2,ipadx=5,ipady=5)
...
red_slider = tkinter.Scale(input_frame,from_=0,to=255,cursor='hand2',command=get_red)
red_button = tkinter.Button(input_frame,text='Red',cursor='hand2',command=lambda:set_color(255,0,0))
green_label = tkinter.Label(input_frame,text='G')
green_slider = tkinter.Scale(input_frame,from_=0,to=255,cursor='hand2',command=get_green)
green_button = tkinter.Button(input_frame,text='Green',cursor='hand2',command=lambda:set_color(0,255,0))
blue_label = tkinter.Label(input_frame,text='B')
blue_slider = tkinter.Scale(input_frame,from_=0,to=255,cursor='hand2',command=get_blue)
blue_button = tkinter.Button(input_frame,text='Blue',cursor='hand2',command=lambda:set_color(0,0,255))

#buttons for complimentary color
yellow_button = tkinter.Button(input_frame,text='Yellow',cursor='hand2',command=lambda:set_color(255,255,0))
cyan_button = tkinter.Button(input_frame,text='Cyan',cursor='hand2',command=lambda:set_color(0,255,255))
magenta_button = tkinter.Button(input_frame,text='Magenta',cursor='hand2',command=lambda:set_color(255,0,255))

#utility buttons
store_button = tkinter.Button(input_frame,text='Store Color',cursor='hand2',command=store_color )
save_button = tkinter.Button(input_frame,text='Save',cursor='hand2',command=save_colors)
quit_button = tkinter.Button(input_frame,text='Close',cursor='hand2',command=root.destroy)
...
blue_button.grid(row=2,column=2,padx=1,pady=1,ipadx=19)
...
#create the colorbox and color labels
...
#radio buttons
for i in range(6):
    radio = tkinter.Radiobutton(output_frame,cursor='hand2',variable=stored_color,value=i)
    radio.grid(row=i,column=0,sticky='W')

    recall_button = tkinter.Button(output_frame,text='Recall Text',cursor='hand2',state=DISABLED)
